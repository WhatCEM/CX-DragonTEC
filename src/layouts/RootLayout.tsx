...
const RoleBadge = ({ accessType }: { accessType?: string }) => {
  const role = ROLE_LABELS[accessType ?? '']
    ?? { label: accessType ?? '—', bg: '#f3f4f6', color: '#374151' };
  return (
    <span style={{
  ...
  return (
    <div className="app-container">
      <header className="app-header">
        ...
        <div className="header-brand"><h1>{appName}</h1></div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          {user && (
            <span style={{
              ...
              {user.name}
              {/* Badge a la DERECHA del nombre, si NO es admin */}
              {!isAdmin && <RoleBadge accessType={user.accessType} />}
            </span>
          )}
