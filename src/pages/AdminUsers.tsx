...
const AdminUsers = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      // ── validar-jwt extrae el token del header "x-token" ──────────────────
      // ── el controlador verifica req.accessType === "admin" ────────────────
      const res = await fetch(`${API_URL}/auth/new`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-token': token ?? '',   // header que usa validarJWT en el back
        },
        ...
        <form onSubmit={handleSubmit} noValidate className="login-form">
          ...
          <div className="lf-field">
            ...
            <div className={`lf-input-wrap ${errors.name ? 'is-error' : ''}`}>
              <input id="au-name" type="text" value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder="Juan Pérez" autoComplete="off" disabled={isLoading}/>
          ...
          <div className="lf-field">
            ...
            <div className={`lf-input-wrap ${errors.email ? 'is-error' : ''}`}>
              <input id="au-email" type="email" value={email}
                onChange={(e) => setEmail(e.target.value)}
                placeholder="nombre@empresa.com" autoComplete="off" disabled={isLoading}/>
              <span className="lf-icon">
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
          ...
          <div className="lf-field">
            ...
            <div className={`lf-input-wrap ${errors.password ? 'is-error' : ''}`}>
              <input id="au-password" type={showPass ? 'text' : 'password'}
                value={password} onChange={(e) => setPassword(e.target.value)}
                placeholder="••••••••" autoComplete="new-password" disabled={isLoading}/>
              <button type="button" className="lf-icon lf-icon--btn"
                onClick={() => setShowPass(v => !v)} tabIndex={-1}>
                {showPass
                  ? <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M2 2L14 14" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/><path d="M4 4.3C2.6 5.3 1.5 6.5 1 8c1.1 3 4 5 7 5 1.4 0 2.7-.4 3.8-1.2" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg>
                  : <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M1 8c1.1-3 4-5 7-5s5.9 2 7 5c-1.1 3-4 5-7 5s-5.9-2-7-5z" stroke="currentColor" strokeWidth="1.3"/><circle cx="8" cy="8" r="2" stroke="currentColor" strokeWidth="1.3"/></svg>}
              </button>
            </div>
          ...
          <div className="lf-field">
            ...
            <div className={`lf-input-wrap ${errors.confirmPassword ? 'is-error' : ''}`}>
              <input id="au-confirm" type={showConfirm ? 'text' : 'password'}
          ...
          <button type="submit" className="lf-submit"
            disabled={isLoading} style={{ marginTop: '0.75rem' }}>
